...
def postings_from_shard(index: str, key: int) -> Optional[List[int]]:
    ...
    Supports both single-level and two-level hex fanout:
      <SHARDS_ROOT>/<index>/<key>.json
      <SHARDS_ROOT>/<index>/<subdir>/<key>.json  (subdir: hex of key bits, e.g. 00..ff / 000..fff)
    """
    base = os.path.join(SHARDS_ROOT, index)
    # Try two-level paths with varying hex widths (4, 3, 2 nibbles)
    for hexw in (4, 3, 2):
        mask = (1 << (hexw * 4)) - 1
        sub = f"{key & mask:0{hexw}x}"
        shard_file = os.path.join(base, sub, f"{key}.json")
        if os.path.exists(shard_file):
            with open(shard_file, "r", encoding="utf-8") as f:
                return json.load(f)
    # Fallback: single-level path
    shard_file = os.path.join(base, f"{key}.json")
    if os.path.exists(shard_file):
        with open(shard_file, "r", encoding="utf-8") as f:
            return json.load(f)
    return None
...
def subtract_years(d: date, years: int) -> date:
    """Return d shifted back by `years`, safe for Feb 29."""
    try:
...
def parse_date_range_text(text: str) -> Optional[Tuple[str, str]]:
    ...
    if m:
        n = max(1, min(200, int(m.group(1))))  # clamp if desired
        today = datetime.now(timezone.utc).date()
        start = subtract_years(today, n)
        return (start.isoformat(), today.isoformat())
...
class SearchFilters(BaseModel):
    ...
    # UX additions:
    sort_by:   Optional[str] = "date"   # "date" | "ad_id" | "company"
    sort_dir:  Optional[str] = "desc"   # "asc" | "desc"
    q_contains: Optional[str] = None    # substring over (company/city/type), Turkish-normalized
...
def search(inp: SearchIn):
    ...
    Perform an indexed search. Strategy:
      ...
      5) NEW: Apply optional q_contains filter, sort hits, then slice to limit.
    """
    f = inp.filters or SearchFilters()
    limit = max(1, min(200, inp.limit or f.limit or 40))

    # Sorting + text filter params
    sort_by  = (getattr(f, "sort_by", "date") or "date").strip().lower()
    sort_dir = (getattr(f, "sort_dir", "desc") or "desc").strip().lower()
    reverse  = (sort_dir != "asc")
    q_norm = norm_tr(f.q_contains) if getattr(f, "q_contains", None) else None
    ...
    # 2.5) Company postings (indexed if available; fallback to post-filter)
    if f.company_code is not None:
        comp_list = postings("comp_code", int(f.company_code))
        if comp_list:
    ...
    for rid in cur:
        ...
        # Optional substring filter against company/city/type labels
        if q_norm:
            lab_company = UNVAN_VOCAB.get(comp_code, "")
            lab_city    = MUDURLUK_NAMES.get(loc, str(loc))
            lab_type    = ILAN_TURU_NAMES.get(typ, str(typ))
            blob = norm_tr(f"{lab_company} {lab_city} {lab_type}")
    ...
    # Sort after filtering; then slice to limit
    def _sort_key(h: dict):
        if sort_by == "ad_id":
            return (h.get("ad_id", 0), h.get("id", -1))
        if sort_by == "company":
            return (norm_tr(h.get("company", "")), h.get("date_int", 0), h.get("id", -1))
        # default: date
        return (h.get("date_int", 0), h.get("id", -1))
