...
POST  /search                     <-- server-side filter/sort/pagination (new)
POST  /answer
...
MUDURLUK_CODES: Dict[str, int] = {}
MUDURLUK_NAMES: Dict[int, str] = {}
ILAN_TURU_CODES: Dict[str, int] = {}
ILAN_TURU_NAMES: Dict[int, str] = {}
UNVAN_VOCAB: Dict[int, str] = {}
UNVAN_NAME_TO_ID: Dict[str, int] = {}
...
class SearchFilters(BaseModel):
    ...
    # NEW: server-side free-text filter (company/city/type/date/ad_id)
    text_contains: Optional[str] = None

class SearchIn(BaseModel):
    filters: SearchFilters
    # NEW: server-side pagination + sorting
    page: int = 1
    page_size: int = 100
...
# FastAPI app + CORS
# ──────────────────────────────────────────────────────────────────────────────
app = FastAPI(title="Helpbot Dataset API", version="1.2.0")
app.add_middleware(
    CORSMiddleware,
...
def search(inp: SearchIn):
    ...
    page_size = max(1, min(200, int(inp.page_size or 100)))  # keep a sane cap
    ...
    # server-side free-text filter (company/city/type/date/ad_id)
    q = norm_tr(f.text_contains or "")
    if q:
        def match_text(h: dict) -> bool:
            if q in norm_tr(h.get("company","")): return True
            if q in norm_tr(h.get("city","")): return True
            if q in norm_tr(h.get("type","")): return True
            if q in str(h.get("ad_id","")): return True
            if q in sec1960_to_iso(h.get("date_int", 0)): return True
            return False
        hits_all = [h for h in hits_all if match_text(h)]
    ...
    keyfn = keymap.get(order_by, keymap["date_int"])
    reverse = (order_dir != "asc")
    hits_all.sort(key=keyfn, reverse=reverse)
...
# ──────────────────────────────────────────────────────────────────────────────
# /answer — summarizer over first N (respects server-side sort)
# ──────────────────────────────────────────────────────────────────────────────
@app.post("/answer")
def answer(inp: AnswerIn):
    s = search(SearchIn(
        filters=inp.filters,
        page=1,
        page_size=min(inp.max_ctx or 20, 100),
        order_by="date_int",
        order_dir="desc"
    ))
    hits = s.get("hits", [])
